...
//	}
	...
	class DataHolder {  
	  ...
	  float parseCoord(String a){
	   ...
	   //for (int i=0;i<c.length;i++) c[i]= match(c[i],"\\d++")[0]; 
	    ...
	    float coord= Integer.valueOf(c[0]) + Integer.valueOf(c[1])*MIN_TO_DEG + 0*SEG_TO_DEG; 
	    println(c[0]+","+c[1]);
	...
	class Globe {
	  ...
	  rWRatio, 
	  rHRatio,
	  ROTATION_FACTOR=(float) (.01*DEG_TO_RAD);
	  PVector   
	  rotation, 
	  rotSpeed;
	  ...
	  void setTexturedSphereDetail(int detail) {   //Set the detail level for textured spheres, constructing the underlying vertex and uv map data  
	    ...
	    float ustep = (float) (.5 / detail); 
	    float vstep = (float) (1. / detail); 
	    int d1= detail+1;
	    int d2= detail*2 +1;
	...
	public void setup(){
	  ...
	  X=  width/2+100;
	  Y= height/2;
	  createBackground (bg,X,Y,(float) .1);
	  frameRate(30);
	  cursor(CROSS);
	  textMode(SCREEN);
	   //Objects
