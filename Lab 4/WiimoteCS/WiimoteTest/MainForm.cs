...
namespace WiimoteTest
{
	...
	{
        bool a   = false; // BOTON A PRESIONADO
        bool hit = false; //GOLPE REALIZADO
        bool hitNunchuk = false; //GOLPE REALIZADO NUnchuk
...
/*            double roll = Math.Atan(-ws.AccelState.Values.X / ws.AccelState.Values.Z);
            roll = (roll * 180) / Math.PI;*/

         //   lblAccel.Text = ws.AccelState.Values.ToString(); /* + " \n ROLL: " + String.Format("{0:00.0}", roll); // roll.ToString();*/
            Wiimote wm = new Wiimote();
